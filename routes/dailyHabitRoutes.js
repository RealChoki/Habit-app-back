...
async function dailyHabitsRoutes(fastify, options) {
  ...
  fastify.delete('/daily-habits/:id', async (request, reply) => {
    try {
      const deletedDailyHabit = await DailyHabit.findByIdAndDelete(request.params.id);

      if (!deletedDailyHabit) {
        return reply.status(404).send({ message: 'Daily Habit not found' });
      }

      reply.status(204).send(); // No content to return
    } catch (error) {
      reply.status(500).send({ message: 'Error deleting daily habit', error });
    }
  });
}
